...
<div class="flex h-full flex-col">
	<Card title="Instruments (Debug)" icon={IconCarbonWaveform} fullHeight={true} class="flex flex-col">
		{#snippet children()}
							...
							{:else}
								<div class="space-y-4">
									{#each song.instruments as instrument}
										<div class="rounded border border-neutral-700 bg-neutral-900/50 p-3">
											<div class="mb-2 flex items-center justify-between border-b border-neutral-800 pb-2">
												...
												<div class="text-xs text-neutral-500">
													Loop: {instrument.loop} | Rows: {instrument.rows.length}
												</div>
											</div>
												...
												<table class="w-full text-xs">
													<thead>
														<tr class="border-b border-neutral-800 text-neutral-400">
															...
															<th class="px-2 py-1 text-left">Mixer</th>
															<th class="px-2 py-1 text-right">ToneAdd</th>
															<th class="px-2 py-1 text-right">NoiseAdd</th>
													...
													<tbody>
														{#each instrument.rows as row, rowIndex}
															{@const isLoopPoint = instrument.loop > 0 && rowIndex === instrument.loop}
															<tr
																class="border-b border-neutral-800/50 {isLoopPoint
																	? 'bg-blue-900/20'
																	: ''}">
																<td class="px-2 py-1 font-mono text-neutral-400">
																	{rowIndex.toString().padStart(2, '0')}
																	{#if isLoopPoint}
																		<span class="ml-1 text-blue-400">↻</span>
																	{/if}
																</td>
																<td class="px-2 py-1 font-mono {row.tone || row.noise || row.envelope
																	? 'text-green-400'
																	: 'text-neutral-600'}">
																	{formatMixerFlags(row)}
																</td>
																<td class="px-2 py-1 text-right font-mono {row.toneAdd !== 0
																	? 'text-yellow-400'
																	: 'text-neutral-500'}">
																	<div class="flex items-center justify-end gap-1">
																		{formatNumber(row.toneAdd, true)}
																		{#if row.toneAccumulation}
																			<span class="text-xs text-blue-400">↑</span>
																		{/if}
																	</div>
																</td>
																<td class="px-2 py-1 text-right font-mono {row.noiseAdd !== 0
																	? 'text-yellow-400'
																	: 'text-neutral-500'}">
																	<div class="flex items-center justify-end gap-1">
																		{formatNumber(row.noiseAdd, true)}
																		{#if row.noiseAccumulation}
																			<span class="text-xs text-blue-400">↑</span>
																		{/if}
																	</div>
																</td>
																<td class="px-2 py-1 text-right font-mono {row.volume > 0
																	? 'text-cyan-400'
																	: 'text-neutral-500'}">
																	<div class="flex items-center justify-end gap-1">
																		{row.volume}
																		{#if row.amplitudeSliding}
																			<span class="text-xs {row.amplitudeSlideUp
																				? 'text-green-400'
																				: 'text-red-400'}">
